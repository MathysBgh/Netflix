    ...
    public ArrayList<Users> recupere() {
        ...
        try {
            ...
            while (resultat.next()) {
                ...
                // Vous pouvez ajouter des méthodes pour gérer les autres attributs de la classe
                // Users (filmLike, filmDispo, historiqueFilm, filmVu, noteFilm, filmEnCours)
    ...
    public void ajoute(Users user) {
        try {
            PreparedStatement statement = con.prepareStatement(
                    "INSERT INTO users (username, password, email, role, pays) VALUES (?, ?, ?, ?, ?)");
    ...
    public void update(Users user) {
        try {
            ...
            try (PreparedStatement statement = con.prepareStatement(
                    "UPDATE users SET username=?, password=?, email=?, role=?, pays=? , profils=? WHERE id=?")) {
                statement.setString(1, user.getUsername());
                statement.setString(2, user.getPassword());
                statement.setString(3, user.getEmail());
                statement.setInt(4, user.getRole());
