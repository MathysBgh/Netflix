...
public class FilmDAOImpl implements FilmDAO {
    ...
    public FilmDAOImpl() {
        try {
            ...
            String user = "root";
            String password = "";
            // String url = "jdbc:mysql://153.92.211.103:3306/testdatabase";
            // String user = "Maxenceusertest";
            // String password = "Maxence@92921";

            this.con = DriverManager.getConnection(url, user, password); // stockage de la connexion dans la variable
                                                                         // d'instance "con"
        } catch (Exception e) {
    ...
    public void ajoute(Film film) {
        try {
            ...
            try (PreparedStatement statement = con.prepareStatement(
                    "INSERT INTO film (titre, realisateur, genre, anne_sortie, synopsis, path_affiche, langue_actu, teaser_titre, quality_video_actu, langue_sous_titre_actu, path_video, path_teaser, duree, acteurs, pays, qualites, Langues, langue_sous_titre, dispo) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)")) {
    ...
    public void update(Film film) {
        try {
            ...
            try (PreparedStatement statement = con.prepareStatement(
                    "UPDATE film SET titre=?, realisateur=?, anne_sortie=?, synopsis=?, path_video=?, acteurs=?, dispo=? WHERE id=?")) {
                // statement.setString(1, film.getTitre());
                // statement.setString(2, film.getRealisateur());
                // statement.setString(3, film.getGenre());
                // statement.setInt(4, film.getDateSortie());
    ...
    // try {
    // try (PreparedStatement statement = con.prepareStatement("UPDATE film SET
    // "+attribut+"=? WHERE id=?")) {
    ...
    // }
    // } catch (SQLException e) {
    // System.out.println("Erreur lors de la modification de l'attribut "+attribut+"
    // pour le film + " + film.getTitre());
    // System.out.println("Modification de l'attribut " + attribut + " pour le film
    // " + film.getTitre() + " terminée");
    // }
